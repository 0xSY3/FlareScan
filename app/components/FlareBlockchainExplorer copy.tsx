...
import FTSOPriceOracle from './FTSOPriceOracle';
import FDCCrossChainExplorer from './FDCCrossChainExplorer';
import FAssetsAnalytics from './FAssetsAnalytics';
import UniversalCrossChainSearch from './UniversalCrossChainSearch';
...
const FlareBlockchainExplorer = () => {
  ...
  const [activeView, setActiveView] = useState<'explorer' | 'dashboard' | 'data-feeds' | 'ftso-oracle' | 'cross-chain' | 'fassets' | 'universal-search'>('explorer');
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex bg-white/80 backdrop-blur-sm rounded-xl p-1 border border-gray-300">
                ...
                <button
                  onClick={() => setActiveView('ftso-oracle')}
                  className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                    activeView === 'ftso-oracle'
                      ? 'bg-gradient-to-r from-pink-600 to-rose-600 text-white shadow-md'
                      : 'text-gray-800 hover:text-pink-600 hover:bg-gray-100'
                  }`}
                >
                ...
                <button
                  onClick={() => setActiveView('cross-chain')}
                  className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${
...
>
  <div className="w-full px-6 py-6">
    {activeView === 'universal-search' ? (
      <div className="max-w-5xl mx-auto">
        <UniversalCrossChainSearch 
          onSearchComplete={(txHash, chain) => {
            setActiveView('explorer');
            setInput(txHash);
            handleSearch(txHash);
          }}
      ...
      <div className="flex items-center gap-4 max-w-7xl mx-auto">
        {/* Back to Explorer button - Outside left */}
        {isSearchMode && (
          <motion.button
            type="button"
            onClick={handleBackToExplorer}
            whileHover={{ scale: 1.05 }}
            className="px-5 py-4 text-gray-800 hover:text-pink-600 font-medium bg-white rounded-xl border border-gray-300 shadow-sm whitespace-nowrap flex items-center gap-2 transition-all"
          >
        ...
        {/* Search Form - Takes up remaining space */}
        <form ref={formRef} onSubmit={handleFormSubmit} className="relative flex-1">
          ...
          <input
            ...
            disabled={isLoading}
            className="w-full pl-16 pr-44 py-5 bg-white border-2 border-gray-300 rounded-2xl focus:outline-none focus:border-pink-500 focus:ring-4 focus:ring-pink-200 shadow-lg text-lg placeholder-gray-600 text-gray-900 transition-all"
          />
          <motion.button 
            type="submit" 
            ...
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="absolute right-3 top-1/2 transform -translate-y-1/2 px-6 py-3 bg-gradient-to-r from-pink-600 to-rose-600 text-white rounded-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 font-medium shadow-lg hover:shadow-xl transition-all whitespace-nowrap"
          >
            {isLoading ? (
              <>
        ...
        {/* Universal Search button - Outside right */}
        <motion.button
          type="button"
          onClick={() => setActiveView('universal-search')}
          whileHover={{ scale: 1.05 }}
          className="px-5 py-4 text-purple-600 hover:text-purple-700 font-medium bg-purple-50 rounded-xl border border-purple-200 shadow-sm whitespace-nowrap flex items-center gap-2 transition-all"
        >
          <Globe className="w-5 h-5" />
          <span className="hidden md:inline">Universal</span> Search
        </motion.button>
      </div>
    )}
    ...
    {error && (
      <motion.div 
        ...
        className="mt-4 max-w-7xl mx-auto flex items-center justify-between p-4 bg-red-50 border border-red-200 rounded-xl"
      >
          ...
          {isSearchMode ? (
            ...
            >
              ...
              {activeView === 'ftso-oracle' && (
                <FTSOPriceOracle 
                  chainId={currentChain.id} 
                  provider={currentClient as any}
                  onPriceUpdate={(feedId, price) => {
                    console.log('Price update:', feedId, price);
                  }}
                />
              )}

              {activeView === 'cross-chain' && (
                <FDCCrossChainExplorer chainId={currentChain.id} />
              )}
